{{-- resources/views/modul.blade.php --}}
<!DOCTYPE html>
<html lang="id">
<head>
    <meta charset="UTF-8">
                ...
                <div class="mb-8">
                    <div class="border border-blue-900 rounded-xl p-4 bg-white shadow">
                        <div class="flex items-start space-x-4">
                            ...
                            {{-- Course Info --}}
                            <div class="flex-1">
                                <h2 class="text-xl font-bold mb-1">{{ $course->name }}</h2>
                                <p class="text-gray-600 text-sm mb-2">{{ $course->description }}</p>
                                <p class="text-black-500 text-sm font-bold">Dibuat Oleh : {{ $course->instructor }}</p>
                                
                                {{-- Progress Bar --}}
                                <div class="mt-4">
                                    <div class="w-full bg-gray-200 rounded-full h-2">
                                        <div class="bg-blue-900 h-2 rounded-full" style="width: {{ $course->progress }}%"></div>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="flex justify-end mt-4">
                            <a href="{{ route('courses.tool', $course->slug) }}" class="bg-blue-900 text-white px-6 py-2 rounded-full text-sm">Lanjutkan</a>
                        </div>
                    </div>
                </div>
                @empty
                <div class="border border-blue-900 rounded-xl p-4 bg-white shadow">
                    <p class="text-center text-gray-600">Belum ada kursus yang sedang berjalan</p>
                </div>
                @endforelse
            </div>
            <!-- Sidebar -->
